		...
		function showForecastChart(dataset,errors){
			...
			nestedData.forEach(function(d) {
                ...
                d.temperature = +d.values[0].value;//d3.format('.0f')(d.values[0].value);// + "°C";
			...
			var	margin = {top: 0, right: 0, bottom: 0, left: 20},//var	margin = {top: 30, right: 20, bottom: 30, left: 50},
			...
			//function update(data, error){
				...
				//y.domain([0, d3.max(nestedData, function(d) { return d.temperture; })]);
				y.domain(d3.extent(nestedData, function(d) { return d.temperature; }));
			 	//y.domain([-35, 35]);
			 	
			 	//Draw main forecast
				//main place
/*				var textPlace = svg.selectAll("h2")
					.data(arrData);
...
/*
				...
				//add tooltip
				var weatherSymbolTemp = svg.selectAll("h1")
				    .data(arrData);
				weatherSymbolTemp.enter().append("h1");
				weatherSymbolTemp.attr("class", function(d) { return "climacon " + symbolMap[d.data.weathersymbol3.timeValuePairs[0].value] ; });
				weatherSymbolTemp.text(function(d){ 
